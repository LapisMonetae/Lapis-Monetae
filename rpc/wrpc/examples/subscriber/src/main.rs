// Basic example of a Lapis Monetae wRPC client that connects to a node
...
// Lapis Monetae RPC primitives
use lmt_wrpc_client::prelude::*;
// reuse wRPC Result type for convenience
use lmt_wrpc_client::result::Result;
...
struct Inner {
    ...
    // Lapis Monetae wRPC client instance
...
impl Listener {
    pub fn try_new(network_id: NetworkId, url: Option<String>) -> Result<Self> {
        ...
        // Create a basic Lapis Monetae RPC client instance using Borsh encoding.
        let client = Arc::new(LmtRpcClient::new_with_args(WrpcEncoding::Borsh, url.as_deref(), resolver, Some(network_id), None)?);
    ...
    async fn start_event_task(&self) -> Result<()> {
        ...
        // in the rusty Lapis Monetae framework)

...
use lmt_consensus_core::{subnets::SubnetworkConversionError, tx::TransactionId, utxo::utxo_inquirer::UtxoInquirerError};
use lmt_utils::networking::IpAddress;
...
pub enum RpcError {
    ...
    #[error(transparent)]
    AddressError(#[from] lmt_addresses::AddressError),

    #[error(transparent)]
    NetworkTypeError(#[from] lmt_consensus_core::network::NetworkTypeError),

    #[error(transparent)]
    NetworkIdError(#[from] lmt_consensus_core::network::NetworkIdError),

    #[error(transparent)]
    NotificationError(#[from] lmt_notify::error::Error),

    #[error(transparent)]
    MiningManagerError(#[from] lmt_mining_errors::manager::MiningManagerError),

    #[error(transparent)]
    ConsensusError(#[from] lmt_consensus_core::errors::consensus::ConsensusError),

    #[error(transparent)]
    ScriptClassError(#[from] lmt_txscript::script_class::Error),

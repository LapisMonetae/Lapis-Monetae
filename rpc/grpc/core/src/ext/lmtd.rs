use lmt_notify::{scope::Scope, subscription::Command};

use crate::protowire::{
    lmtd_request, lmtd_response, LmtdRequest, LmtdResponse, NotifyBlockAddedRequestMessage, NotifyFinalityConflictRequestMessage,
...
impl LmtdRequest {
    pub fn from_notification_type(scope: &Scope, command: Command) -> Self {
        LmtdRequest { id: 0, payload: Some(lmtd_request::Payload::from_notification_type(scope, command)) }
    }
...
impl lmtd_request::Payload {
    pub fn from_notification_type(scope: &Scope, command: Command) -> Self {
        match scope {
            Scope::BlockAdded(_) => {
                lmtd_request::Payload::NotifyBlockAddedRequest(NotifyBlockAddedRequestMessage { command: command.into() })
            }
            Scope::NewBlockTemplate(_) => {
                lmtd_request::Payload::NotifyNewBlockTemplateRequest(NotifyNewBlockTemplateRequestMessage { command: command.into() })
            }

            Scope::VirtualChainChanged(ref scope) => {
                lmtd_request::Payload::NotifyVirtualChainChangedRequest(NotifyVirtualChainChangedRequestMessage {
                    command: command.into(),
                    include_accepted_transaction_ids: scope.include_accepted_transaction_ids,
                })
            }
            Scope::FinalityConflict(_) => {
                lmtd_request::Payload::NotifyFinalityConflictRequest(NotifyFinalityConflictRequestMessage { command: command.into() })
            }
            Scope::FinalityConflictResolved(_) => {
                lmtd_request::Payload::NotifyFinalityConflictRequest(NotifyFinalityConflictRequestMessage { command: command.into() })
            }
            Scope::UtxosChanged(ref scope) => lmtd_request::Payload::NotifyUtxosChangedRequest(NotifyUtxosChangedRequestMessage {
                addresses: scope.addresses.iter().map(|x| x.into()).collect::<Vec<String>>(),
                command: command.into(),
            }),
            Scope::SinkBlueScoreChanged(_) => {
                lmtd_request::Payload::NotifySinkBlueScoreChangedRequest(NotifySinkBlueScoreChangedRequestMessage {
                    command: command.into(),
                })
            }
            Scope::VirtualDaaScoreChanged(_) => {
                lmtd_request::Payload::NotifyVirtualDaaScoreChangedRequest(NotifyVirtualDaaScoreChangedRequestMessage {
                    command: command.into(),
                })
            }
            Scope::PruningPointUtxoSetOverride(_) => {
                lmtd_request::Payload::NotifyPruningPointUtxoSetOverrideRequest(NotifyPruningPointUtxoSetOverrideRequestMessage {
                    command: command.into(),
                })
            }
        }
    ...
    pub fn is_subscription(&self) -> bool {
        use crate::protowire::lmtd_request::Payload;
        matches!(
            self,
...
impl lmtd_response::Payload {
    pub fn is_notification(&self) -> bool {
        use crate::protowire::lmtd_response::Payload;
        match self {

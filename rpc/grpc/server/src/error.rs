...
pub enum GrpcServerError {
    ...
    RpcApiError(#[from] lmt_rpc_core::error::RpcError),
...
impl From<GrpcServerError> for lmt_rpc_core::error::RpcError {
    fn from(err: GrpcServerError) -> Self {
        match err {
            GrpcServerError::RpcApiError(err) => err,
            GrpcServerError::NotificationError(err) => err.into(),
            _ => lmt_rpc_core::error::RpcError::General(err.to_string()),
        }
    }
}

impl From<GrpcServerError> for lmt_notify::error::Error {
    fn from(err: GrpcServerError) -> Self {
        match err {
            GrpcServerError::RpcApiError(err) => lmt_notify::error::Error::General(err.to_string()),
            GrpcServerError::NotificationError(err) => err,
            _ => lmt_notify::error::Error::General(err.to_string()),
        }
    }
}
...
impl<T> From<TrySendError<T>> for GrpcServerError {
    fn from(_: TrySendError<T>) -> Self {
        lmt_notify::error::Error::ChannelSendError.into()
    }
}

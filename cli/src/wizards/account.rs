...
// use lmt_wallet_core::runtime::wallet::AccountCreateArgsBip32;
// use lmt_wallet_core::runtime::{PrvKeyDataArgs, PrvKeyDataCreateArgs};
// use lmt_wallet_core::storage::AccountKind;
...
async fn create_multisig(ctx: &Arc<LmtCli>, account_name: Option<String>, mnemonic_phrase_word_count: WordCount) -> Result<()> {
    let term = ctx.term();
    let wallet = ctx.wallet();
...
pub(crate) async fn bip32_watch(ctx: &Arc<LmtCli>, name: Option<&str>) -> Result<()> {
    let term = ctx.term();
    let wallet = ctx.wallet();
...
pub(crate) async fn multisig_watch(ctx: &Arc<LmtCli>, name: Option<&str>) -> Result<()> {
    let term = ctx.term();

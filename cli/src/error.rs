...
use lmt_wallet_core::error::Error as WalletError;
...
pub enum Error {
    ...
    #[error(transparent)]
    WrpcError(#[from] lmt_wrpc_client::error::Error),

    #[error(transparent)]
    RpcError(#[from] lmt_rpc_core::RpcError),
    ...
    #[error(transparent)]
    Daemon(#[from] lmt_daemon::error::Error),
    ...
    #[error(transparent)]
    Bip32(#[from] lmt_bip32::Error),

    #[error("private key {0} already exists")]
    PrivateKeyAlreadyExists(String),

    #[error(transparent)]
    MetricsError(lmt_metrics_core::error::Error),

    #[error(transparent)]
    LmtWalletKeys(#[from] lmt_wallet_keys::error::Error),

    #[error(transparent)]
    PskbLockScriptSigError(#[from] lmt_wallet_pskt::error::Error),

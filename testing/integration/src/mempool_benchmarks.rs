...
use lmt_consensus_core::{constants::SOMPI_PER_LMT, network::NetworkType, tx::Transaction};
use lmt_core::{debug, info};
use lmt_notify::{
    listener::ListenerId,
    scope::{NewBlockTemplateScope, Scope},
};
use lmt_rpc_core::{api::rpc::RpcApi, Notification, RpcError};
use lmt_txscript::pay_to_address_script;
use lmt_utils::fd_budget;
...
/// `cargo test --release --package lmt-testing-integration --lib --features devnet-prealloc -- mempool_benchmarks::bench_bbt_latency --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
async fn bench_bbt_latency() {
    lmt_core::log::try_init_logger("info,lmt_core::time=debug,lmt_mining::monitor=debug");
    ...
    */
    ...
    let args = Args {
        ...
        prealloc_amount: 500 * SOMPI_PER_LMT,
    ...
    let pay_address =
        Address::new(network.network_type().into(), lmt_addresses::Version::PubKey, &pk.x_only_public_key().0.serialize());
    ...
    let submit_block_pool_tasks = submit_block_pool.start(|c, block| async move {
        let _sw = lmt_core::time::Stopwatch::<500>::with_threshold("sb");
        let response = c.submit_block(block, false).await.unwrap();
        assert_eq!(response.report, lmt_rpc_core::SubmitBlockReport::Success);
        false
    });
    ...
    let submit_tx_pool_tasks = submit_tx_pool.start(|c, (i, tx)| async move {
        match c.submit_transaction(tx.as_ref().into(), false).await {
            ...
            Err(RpcError::General(msg)) if msg.contains("orphan") => {
                lmt_core::warn!("\n\n\n{msg}\n\n");
                lmt_core::warn!("Submitted {} transactions, exiting tx submit loop", i);
                return true;
            }
            Err(e) => panic!("{e}"),
        }
        ...
        while let Ok(notification) = notification_rx.recv().await {
            match notification {
                Notification::NewBlockTemplate(_) => {
                    ...
                    // let _sw = lmt_core::time::Stopwatch::<500>::with_threshold("bbt");
                    *current_template.lock() = cc.get_block_template(pac.clone(), vec![]).await.unwrap();
                }
                _ => panic!(),
            }
            if !exec.load(Ordering::Relaxed) {
                lmt_core::warn!("Test is over, stopping miner receiver loop");
                break;
            }
        }
        lmt_core::warn!("Miner receiver loop task exited");
    });
    ...
    let miner_loop_task = tokio::spawn(async move {
        for i in 0..BLOCK_COUNT {
            ...
            if !exec.load(Ordering::Relaxed) {
                lmt_core::warn!("Test is over, stopping miner loop");
                break;
            }
        }
        ...
        lmt_core::warn!("Miner loop task exited");
    });
    ...
    let tx_sender_task = tokio::spawn(async move {
        ...
        lmt_core::warn!("Tx sender task, waiting for mempool to drain..");
        loop {
            if !exec.load(Ordering::Relaxed) {
                break;
            }
        ...
        exec.store(false, Ordering::Relaxed);
        lmt_core::warn!("Tx sender task exited");
    });
...
/// Run this benchmark with the following command line:
/// `cargo test --release --package lmt-testing-integration --lib --features devnet-prealloc -- mempool_benchmarks::bench_bbt_latency_2 --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]

...
use lmt_consensus_core::hashing::sighash_type::SigHashType;
use lmt_consensus_core::tx::{ScriptPublicKey, TransactionInput, UtxoEntry, VerifiableTransaction};
use lmt_txscript_errors::TxScriptError;
use log::trace;
...
impl<'a, T: VerifiableTransaction, Reused: SigHashReusedValues> TxScriptEngine<'a, T, Reused> {
    ...
    fn execute_opcode(&mut self, opcode: DynOpcodeImplementation<T, Reused>) -> Result<(), TxScriptError> {
        // Different from lmtd: Illegal and disabled opcode are checked on execute instead
        // Note that this includes OP_RESERVED which counts as a push operation.
...
mod tests {
    ...
    use lmt_consensus_core::hashing::sighash::SigHashReusedValuesUnsync;
    use lmt_consensus_core::hashing::sighash_type::SIG_HASH_ALL;
...
mod bitcoind_tests {
    ...
    use lmt_consensus_core::constants::MAX_TX_IN_SEQUENCE_NUM;
    use lmt_consensus_core::hashing::sighash::SigHashReusedValuesUnsync;
    use lmt_consensus_core::tx::{

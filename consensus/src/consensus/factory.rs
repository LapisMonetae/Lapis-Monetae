...
use lmt_consensus_core::{config::Config, mining_rules::MiningRules};
use lmt_consensus_notify::root::ConsensusNotificationRoot;
use lmt_consensusmanager::{ConsensusFactory, ConsensusInstance, DynConsensusCtl, SessionLock};
use lmt_core::{debug, time::unix_now, warn};
use lmt_database::{
    prelude::{
...
impl ConsensusFactory for Factory {
    fn new_active_consensus(&self) -> (ConsensusInstance, DynConsensusCtl) {
        ...
        let db = lmt_database::prelude::ConnBuilder::default()
            .with_db_path(dir)

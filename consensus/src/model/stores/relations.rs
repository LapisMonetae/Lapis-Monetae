...
use lmt_consensus_core::BlockHashSet;
use lmt_consensus_core::{blockhash::BlockHashes, BlockHashMap, BlockHasher, BlockLevel};
use lmt_database::prelude::{BatchDbWriter, CachePolicy, DbWriter};
use lmt_database::prelude::{CachedDbAccess, DbKey, DirectDbWriter};
use lmt_database::prelude::{DirectWriter, MemoryWriter};
use lmt_database::prelude::{ReadLock, StoreError};
use lmt_database::prelude::{StoreResult, DB};
use lmt_database::registry::{DatabaseStorePrefixes, SEPARATOR};
use lmt_hashes::Hash;
...
impl RelationsStoreReader for StagingRelationsStore<'_> {
    ...
    fn counts(&self) -> Result<(usize, usize), StoreError> {
        let count = self
            ...
            .iterator()
            .map(|r| r.unwrap().0)
            .map(|k| <[u8; lmt_hashes::HASH_SIZE]>::try_from(&k[..]).unwrap())
            .map(Hash::from_bytes)
...
mod tests {
    ...
    fn test_db_relations_store() {
        let (lt, db) = create_temp_db!(lmt_database::prelude::ConnBuilder::default().with_files_limit(10));

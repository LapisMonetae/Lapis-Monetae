...
use lmt_consensus_core::{blockhash::ORIGIN, header::Header, BlockHashMap, BlockHasher, BlockLevel};
use lmt_hashes::Hash;
...
mod tests {
    ...
    use lmt_database::prelude::{ReadLock, StoreError, StoreResult};
    use lmt_hashes::Hash;
    ...
    impl HeaderStoreReader for HeaderStoreMock {
        fn get_daa_score(&self, hash: lmt_hashes::Hash) -> Result<u64, StoreError> {
            unimplemented!()
        }

        fn get_timestamp(&self, hash: lmt_hashes::Hash) -> Result<u64, StoreError> {
            unimplemented!()
        }

        fn get_bits(&self, hash: lmt_hashes::Hash) -> Result<u32, StoreError> {
            unimplemented!()
        }

        fn get_header(&self, hash: lmt_hashes::Hash) -> Result<Arc<Header>, StoreError> {
            Ok(self.map.read().get(&hash).unwrap().header.clone())
        }

        fn get_compact_header_data(
            &self,
            hash: lmt_hashes::Hash,
        ) -> Result<crate::model::stores::headers::CompactHeaderData, StoreError> {
            unimplemented!()
        }

        fn get_blue_score(&self, hash: lmt_hashes::Hash) -> Result<u64, StoreError> {
            unimplemented!()
        }

        fn get_header_with_block_level(&self, hash: lmt_hashes::Hash) -> Result<HeaderWithBlockLevel, StoreError> {
    ...
    impl RelationsStoreReader for RelationsStoreMock {
        fn get_parents(&self, hash: Hash) -> Result<lmt_consensus_core::blockhash::BlockHashes, StoreError> {
            unimplemented!()
        }

...
use lmt_core::debug;
use lmt_database::prelude::{CachePolicy, ConnBuilder, StoreError, StoreResult, StoreResultEmptyTuple, StoreResultExtensions, DB};
use lmt_hashes::Hash;
...
impl<T: RelationsStoreReader, U: ReachabilityService> RelationsStoreReader for RelationsStoreInFutureOfRoot<T, U> {
    fn get_parents(&self, hash: Hash) -> Result<BlockHashes, lmt_database::prelude::StoreError> {
    ...
    fn get_children(&self, hash: Hash) -> StoreResult<lmt_database::prelude::ReadLock<BlockHashSet>> {
    ...
    fn counts(&self) -> Result<(usize, usize), lmt_database::prelude::StoreError> {
        unimplemented!()
    }
}
...
impl PruningProofManager {
    pub(crate) fn build_pruning_point_proof(&self, pp: Hash) -> PruningPointProof {
        ...
        let (_db_lifetime, temp_db) = lmt_database::create_temp_db!(ConnBuilder::default().with_files_limit(10));

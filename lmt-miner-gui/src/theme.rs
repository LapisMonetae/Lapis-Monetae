...
pub enum Icon {
    Wallet, Lock, Send, Transfer, History, Contacts, Node, Config,
    Plus, Import, Play, Stop, Copy, Browse, Terminal, Chart, Help,
    Bridge, Mining, Shield, Eye, Refresh, Check, Warning, Globe, Key,
}

pub fn icon(ui: &mut egui::Ui, i: Icon, sz: f32, col: Color32) {
    let (r, _) = ui.allocate_exact_size(Vec2::splat(sz), egui::Sense::hover());
    if !ui.is_rect_visible(r) { return; }
    ...
    match i {
        ...
        Icon::Contacts => {
            ...
            p.rect_filled(egui::Rect::from_min_max(Pos2::new(c.x-s*0.55, c.y+s*0.12), Pos2::new(c.x+s*0.55, c.y+s*0.65)), CornerRadius{nw:4,ne:4,sw:0,se:0}, col);
        }
        ...
        Icon::Import => {
            ...
            p.line_segment([Pos2::new(c.x, c.y+s*0.25), Pos2::new(c.x+s*0.28, c.y)], Stroke::new(2.0, col));
            p.line_segment([Pos2::new(c.x-s*0.5, c.y+s*0.55), Pos2::new(c.x+s*0.5, c.y+s*0.55)], Stroke::new(2.0, col));
        }
        Icon::Play => {
            let pts = vec![
                Pos2::new(c.x-s*0.35, c.y-s*0.5), Pos2::new(c.x+s*0.5, c.y), Pos2::new(c.x-s*0.35, c.y+s*0.5),
            ];
        ...
        Icon::Browse => {
            ...
            p.rect_filled(egui::Rect::from_min_size(bx.left_top()-Vec2::new(0.0, s*0.22), Vec2::new(s*0.55, s*0.25)), CornerRadius{nw:2,ne:2,sw:0,se:0}, col);
        }
        Icon::Terminal => {
            let bx = egui::Rect::from_center_size(c, Vec2::new(s*1.5, s*1.15));
            p.rect(bx, CornerRadius::same(3), Color32::TRANSPARENT, Stroke::new(1.8, col), StrokeKind::Outside);
            p.line_segment([Pos2::new(bx.left()+s*0.18, c.y-s*0.15), Pos2::new(bx.left()+s*0.48, c.y+s*0.05)], Stroke::new(1.8, col));
            p.line_segment([Pos2::new(bx.left()+s*0.48, c.y+s*0.05), Pos2::new(bx.left()+s*0.18, c.y+s*0.25)], Stroke::new(1.8, col));
            p.line_segment([Pos2::new(c.x-s*0.05, c.y+s*0.25), Pos2::new(c.x+s*0.35, c.y+s*0.25)], Stroke::new(1.8, col));
        }
        Icon::Chart => {
            ...
            for (i, h) in [0.45, 1.0, 0.65].iter().enumerate() {
                ...
                let ht = s * h * 1.1;
                p.rect_filled(egui::Rect::from_min_size(Pos2::new(x, base_y-ht), Vec2::new(bw, ht)), CornerRadius{nw:2,ne:2,sw:0,se:0}, col);
            }
        }
        Icon::Help => {
        ...
        Icon::Mining => {
            p.line_segment([Pos2::new(c.x-s*0.5, c.y+s*0.5), Pos2::new(c.x+s*0.25, c.y-s*0.25)], Stroke::new(2.5, col));
            p.line_segment([Pos2::new(c.x+s*0.25, c.y-s*0.25), Pos2::new(c.x+s*0.55, c.y-s*0.55)], Stroke::new(2.5, col));
            p.line_segment([Pos2::new(c.x+s*0.25, c.y-s*0.55), Pos2::new(c.x+s*0.55, c.y-s*0.25)], Stroke::new(2.0, col));
        }
        Icon::Shield => {
            let pts = vec![
        ...
        Icon::Check => {
            ...
            p.line_segment([Pos2::new(c.x-s*0.1, c.y+s*0.3), Pos2::new(c.x+s*0.45, c.y-s*0.35)], Stroke::new(2.5, col));
        }
        Icon::Warning => {
            let pts = vec![
                Pos2::new(c.x, c.y-s*0.6), Pos2::new(c.x+s*0.6, c.y+s*0.5), Pos2::new(c.x-s*0.6, c.y+s*0.5),
            ];
            p.add(egui::Shape::convex_polygon(pts, Color32::TRANSPARENT, Stroke::new(1.8, col)));
        ...
        Icon::Key => {
            ...
            p.line_segment([Pos2::new(c.x+s*0.55, c.y+s*0.05), Pos2::new(c.x+s*0.55, c.y+s*0.25)], Stroke::new(1.8, col));
            p.line_segment([Pos2::new(c.x+s*0.35, c.y+s*0.05), Pos2::new(c.x+s*0.35, c.y+s*0.2)], Stroke::new(1.5, col));
        }
    }
}

// ── Text helpers ──────────────────────────────────────────────────────
pub fn heading(t: &str) -> RichText { RichText::new(t).font(FontId::proportional(22.0)).color(TEXT_PRIMARY).strong() }
pub fn subheading(t: &str) -> RichText { RichText::new(t).font(FontId::proportional(15.0)).color(TEXT_SECONDARY).strong() }
pub fn section_title(t: &str) -> RichText { RichText::new(t).font(FontId::proportional(14.0)).color(TEXT_PRIMARY).strong() }
pub fn label_text(t: &str) -> RichText { RichText::new(t).font(FontId::proportional(12.0)).color(TEXT_SECONDARY).strong() }
pub fn body_text(t: &str) -> RichText { RichText::new(t).font(FontId::proportional(14.0)).color(TEXT_PRIMARY) }
pub fn mono(t: &str) -> RichText { RichText::new(t).font(FontId::monospace(13.0)).color(TEXT_PRIMARY) }
pub fn mono_term(t: &str) -> RichText { RichText::new(t).font(FontId::monospace(12.0)).color(TERMINAL_TEXT) }
pub fn big_number(t: &str, col: Color32) -> RichText { RichText::new(t).font(FontId::proportional(28.0)).color(col).strong() }
...
pub fn btn_primary(ui: &mut egui::Ui, txt: &str) -> egui::Response {
    ui.add(egui::Button::new(RichText::new(txt).font(FontId::proportional(13.0)).color(TEXT_WHITE).strong())
        .fill(BLUE).corner_radius(CornerRadius::same(8)).min_size(Vec2::new(110.0, 36.0)))
}
pub fn btn_success(ui: &mut egui::Ui, txt: &str) -> egui::Response {
    ui.add(egui::Button::new(RichText::new(txt).font(FontId::proportional(13.0)).color(TEXT_WHITE).strong())
        .fill(GREEN).corner_radius(CornerRadius::same(8)).min_size(Vec2::new(110.0, 36.0)))
}
pub fn btn_warning(ui: &mut egui::Ui, txt: &str) -> egui::Response {
    ui.add(egui::Button::new(RichText::new(txt).font(FontId::proportional(13.0)).color(TEXT_WHITE).strong())
        .fill(ORANGE).corner_radius(CornerRadius::same(8)).min_size(Vec2::new(110.0, 36.0)))
}
pub fn btn_danger(ui: &mut egui::Ui, txt: &str) -> egui::Response {
    ui.add(egui::Button::new(RichText::new(txt).font(FontId::proportional(13.0)).color(TEXT_WHITE).strong())
        .fill(RED).corner_radius(CornerRadius::same(8)).min_size(Vec2::new(110.0, 36.0)))
}
pub fn btn_secondary(ui: &mut egui::Ui, txt: &str) -> egui::Response {
    ui.add(egui::Button::new(RichText::new(txt).font(FontId::proportional(13.0)).color(TEXT_SECONDARY).strong())
        .fill(BG_INPUT).stroke(Stroke::new(1.0, BORDER)).corner_radius(CornerRadius::same(8)).min_size(Vec2::new(90.0, 36.0)))
}
pub fn btn_small(ui: &mut egui::Ui, txt: &str, col: Color32) -> egui::Response {
    ui.add(egui::Button::new(RichText::new(txt).font(FontId::proportional(11.0)).color(TEXT_WHITE))
        .fill(col).corner_radius(CornerRadius::same(6)).min_size(Vec2::new(65.0, 26.0)))
}
pub fn btn_icon(ui: &mut egui::Ui, ico: Icon, txt: &str, col: Color32) -> egui::Response {
    let r = ui.horizontal(|ui| {
        icon(ui, ico, 16.0, TEXT_WHITE);
        ui.add(egui::Button::new(RichText::new(txt).font(FontId::proportional(13.0)).color(TEXT_WHITE).strong())
            .fill(col).corner_radius(CornerRadius::same(8)).min_size(Vec2::new(90.0, 36.0)))
    });
    r.inner
}
...
pub fn alert_error(ui: &mut egui::Ui, text: &str) {
    egui::Frame::new().fill(RED_BG).stroke(Stroke::new(1.0, RED.linear_multiply(0.3))).corner_radius(CornerRadius::same(8)).inner_margin(Margin::same(12))
        .show(ui, |ui| { ui.horizontal(|ui| { icon(ui, Icon::Warning, 16.0, RED); ui.label(RichText::new(text).font(FontId::proportional(13.0)).color(RED)); }); });
}
pub fn alert_success(ui: &mut egui::Ui, text: &str) {
    egui::Frame::new().fill(GREEN_BG).stroke(Stroke::new(1.0, GREEN.linear_multiply(0.3))).corner_radius(CornerRadius::same(8)).inner_margin(Margin::same(12))
        .show(ui, |ui| { ui.horizontal(|ui| { icon(ui, Icon::Check, 16.0, GREEN); ui.label(RichText::new(text).font(FontId::proportional(13.0)).color(GREEN)); }); });
}
pub fn alert_warning(ui: &mut egui::Ui, text: &str) {
    egui::Frame::new().fill(ORANGE_BG).stroke(Stroke::new(1.0, ORANGE.linear_multiply(0.3))).corner_radius(CornerRadius::same(8)).inner_margin(Margin::same(12))
        .show(ui, |ui| { ui.horizontal(|ui| { icon(ui, Icon::Warning, 16.0, ORANGE); ui.label(RichText::new(text).font(FontId::proportional(13.0)).color(ORANGE)); }); });
}
pub fn alert_info(ui: &mut egui::Ui, text: &str) {
    egui::Frame::new().fill(BLUE_BG).stroke(Stroke::new(1.0, BLUE.linear_multiply(0.3))).corner_radius(CornerRadius::same(8)).inner_margin(Margin::same(12))
        .show(ui, |ui| { ui.horizontal(|ui| { icon(ui, Icon::Help, 16.0, BLUE); ui.label(RichText::new(text).font(FontId::proportional(13.0)).color(BLUE)); }); });
}

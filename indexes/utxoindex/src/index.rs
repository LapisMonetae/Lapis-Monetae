...
use lmt_consensus_core::{tx::ScriptPublicKeys, utxo::utxo_diff::UtxoDiff, BlockHashSet};
use lmt_consensusmanager::{ConsensusManager, ConsensusResetHandler};
use lmt_core::{info, trace};
use lmt_database::prelude::{StoreError, StoreResult, DB};
use lmt_hashes::Hash;
...
const RESYNC_CHUNK_SIZE: usize = 2048; //Increased from 1k (used in go-lmtdd), for quicker resets, while still having a low memory footprint.

/// UtxoIndex indexes `CompactUtxoEntryCollections` by [`ScriptPublicKey`](lmt_consensus_core::tx::ScriptPublicKey),
...
impl UtxoIndexApi for UtxoIndex {
    ...
    fn get_all_outpoints(&self) -> StoreResult<std::collections::HashSet<lmt_consensus_core::tx::TransactionOutpoint>> {

...
pub fn get_lmt_tempdir() -> TempDir {
    let global_tempdir = std::env::temp_dir();
    let lmt_tempdir = global_tempdir.join("lapis-monetae");
    std::fs::create_dir_all(lmt_tempdir.as_path()).unwrap();
    let db_tempdir = tempfile::tempdir_in(lmt_tempdir.as_path()).unwrap();
    db_tempdir
}

/// Creates a DB within a temp directory under `<OS SPECIFIC TEMP DIR>/lapis-monetae`
/// Callers must keep the `TempDbLifetime` guard for as long as they wish the DB to exist.
#[macro_export]
macro_rules! create_temp_db {
    ($conn_builder: expr) => {{
        let db_tempdir = $crate::utils::get_lmt_tempdir();

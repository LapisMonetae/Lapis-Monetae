...
use lmt_consensus_core::{
    ...
    constants::{SOMPI_PER_LMT, TX_VERSION},
    sign::sign,
    subnets::SUBNETWORK_ID_NATIVE,
    tx::{MutableTransaction, Transaction, TransactionInput, TransactionOutpoint, TransactionOutput, UtxoEntry},
};
use lmt_core::{info, lmtd_env::version, time::unix_now, warn};
use lmt_grpc_client::{ClientPool, GrpcClient};
use lmt_notify::subscription::context::SubscriptionContext;
use lmt_rpc_core::{api::rpc::RpcApi, notify::mode::NotificationMode, RpcUtxoEntry};
...
const DEFAULT_SEND_AMOUNT: u64 = 10 * SOMPI_PER_LMT;
...
async fn main() {
    ...
    let schnorr_key = if let Some(private_key_hex) = args.private_key {
        ...
        let lmt_addr = Address::new(ADDRESS_PREFIX, ADDRESS_VERSION, &pk.x_only_public_key().0.serialize());
        info!(
            "Generated private key {} and address {}. Send some funds to this address and rerun rothschild with `--private-key {}`",
            sk.display_secret(),
            String::from(&lmt_addr),
            sk.display_secret()
        );
        return;
    };

    let lmt_addr = Address::new(ADDRESS_PREFIX, ADDRESS_VERSION, &schnorr_key.x_only_public_key().0.serialize());

    let lmt_to_addr = args.addr.as_ref().map_or_else(|| lmt_addr.clone(), |addr_str| Address::try_from(addr_str.clone()).unwrap());
    ...
    if args.addr.is_some() {
        log_message.push_str(&format!("\n\tto address: {}", String::from(&lmt_to_addr)));
    }
...
\tMedian time: {}, \n\tDAA score: {}, \n\tPruning point: {}, \n\tTips: {}, \n\t{} virtual parents: ...{}, \n\tCoinbase maturity: {}",
    ...
    let mut utxos = refresh_utxos(&rpc_client, lmt_addr.clone(), &mut pending, coinbase_maturity).await;
    ...
    loop {
        ...
        if !has_funds || now - last_refresh > 60_000 {
            ...
            utxos = refresh_utxos(&rpc_client, lmt_addr.clone(), &mut pending, coinbase_maturity).await;
...
async fn refresh_utxos(
    ...
    populate_pending_outpoints_from_mempool(rpc_client, lmt_addr.clone(), pending).await;
    fetch_spendable_utxos(rpc_client, lmt_addr, coinbase_maturity, pending).await
}
...
async fn populate_pending_outpoints_from_mempool(
    ...
    let entries = rpc_client.get_mempool_entries_by_addresses(vec![lmt_addr], true, false).await.unwrap();
    let now = Instant::now();
...
async fn fetch_spendable_utxos(
    ...
    let resp = rpc_client.get_utxos_by_addresses(vec![lmt_addr]).await.unwrap();
    ...
    let mut utxos = resp.into_iter()
        ...
        .collect::<Vec<_>>();
    utxos.sort_by_key(|entry| std::cmp::Reverse(entry.1.amount));
    utxos
}
    ...
    let txs = selected_utxos_groups
        ...
        .map(|utxo_option| {
            if let Some((selected_utxos, selected_amount)) = utxo_option {
                let tx = generate_tx(schnorr_key, &selected_utxos, selected_amount, num_outs, &lmt_addr, tx_config.payload_size);
...
fn generate_tx(
    ...
    let script_public_key = pay_to_address_script(lmt_addr);
    let inputs = utxos
        .iter()

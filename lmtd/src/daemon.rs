...
use lmt_consensus_notify::{root::ConsensusNotificationRoot, service::NotifyService};
use lmt_core::{core::Core, debug, info, trace};
use lmt_core::{lmtd_env::version, task::tick::TickService};
use lmt_database::{
    prelude::{CachePolicy, DbWriter, DirectDbWriter},
    registry::DatabaseStorePrefixes,
};
use lmt_grpc_server::service::GrpcService;
use lmt_notify::{address::tracker::Tracker, subscription::context::SubscriptionContext};
use lmt_p2p_lib::Hub;
use lmt_p2p_mining::rule_engine::MiningRuleEngine;
use lmt_rpc_service::service::RpcCoreService;
use lmt_txscript::caches::TxScriptCacheCounters;
use lmt_utils::git;
use lmt_utils::networking::ContextualNetAddress;
use lmt_utils::sysinfo::SystemInfo;
use lmt_utils_tower::counters::TowerConnectionCounters;

use lmt_addressmanager::AddressManager;
use lmt_consensus::{consensus::factory::Factory as ConsensusFactory, pipeline::ProcessingCounters};
use lmt_consensus::{
...
use lmt_p2p_flows::{flow_context::FlowContext, service::P2pService};

use itertools::Itertools;
use lmt_perf_monitor::{builder::Builder as PerfMonitorBuilder, counters::CountersSnapshot};
use lmt_utxoindex::{api::UtxoIndexProxy, UtxoIndex};
use lmt_wrpc_server::service::{Options as WrpcServerOptions, WebSocketCounters as WrpcServerCounters, WrpcEncoding, WrpcService};

/// Desired soft FD limit that needs to be configured
/// for the lmtd process.
pub const DESIRED_DAEMON_SOFT_FD_LIMIT: u64 = 8 * 1024;
/// Minimum acceptable soft FD limit for the lmtd
/// process. (Lapis Monetae will operate with the minimal
...
pub fn get_app_dir() -> PathBuf {
    #[cfg(target_os = "windows")]
    return get_home_dir().join("lapis-monetae");
    #[cfg(not(target_os = "windows"))]
    return get_home_dir().join(".lapis-monetae");
}
...
/// the application folder that contains lmtd logs and the database.
    ...
    pub fn from_args(args: &Args) -> Self {
        ...
        cfg_if::cfg_if! {
            if #[cfg(feature = "semaphore-trace")] {
                lmt_core::log::init_logger(log_dir.as_deref(), &format!("{},{}=debug", args.log_level, lmt_utils::sync::semaphore_module_path()));
            } else {
                lmt_core::log::init_logger(log_dir.as_deref(), &args.log_level);
            }
        };
...
do you confirm? (answer y/n or pass --yes to the lmtd command line to confirm all interactive questions)";
    ...
    let mut meta_db = lmt_database::prelude::ConnBuilder::default()
    ...
    if !is_db_reset_needed && (args.testnet || args.devnet || args.simnet) {
        ...
        match active_consensus_dir_name {
            Some(dir_name) => {
                let consensus_db = lmt_database::prelude::ConnBuilder::default()
    ...
    // Reset Condition: Need to reset if we're upgrading from a previous DB version
            ...
            || MultiConsensusManagementStore::new(meta_db.clone()).should_upgrade().unwrap())
    {
        ...
        if version == 3 {
            ...
            match active_consensus_dir_name {
                ...
                None => {
                    let msg =
                    "Node database is from a different LMTD *DB* version and needs to be fully deleted, do you confirm the delete? (y/n)";
                    get_user_approval_or_exit(msg, args.yes);
            ...
            info!("Deleting databases from previous LMTD version");
    ...
    if is_db_reset_needed && !args.reset_db {
        ...
        // Reopen the DB
        meta_db = lmt_database::prelude::ConnBuilder::default()
    ...
    let perf_monitor = if args.perf_metrics {
        let cb = move |counters: CountersSnapshot| {
            debug!("[{}] {}", lmt_perf_monitor::SERVICE_NAME, counters.to_process_metrics_display());
            debug!("[{}] {}", lmt_perf_monitor::SERVICE_NAME, counters.to_io_metrics_display());
            #[cfg(feature = "heap")]
            debug!("[{}] heap stats: {:?}", lmt_perf_monitor::SERVICE_NAME, dhat::HeapStats::get());
        };
    ...
    let index_service: Option<Arc<IndexService>> = if args.utxoindex {
        ...
        let utxoindex_db = lmt_database::prelude::ConnBuilder::default()

...
impl TryFrom<LmtdConfig> for Vec<String> {
    type Error = Error;
    fn try_from(args: LmtdConfig) -> Result<Vec<String>> {
        let mut argv = Vec::new();

        if args.path.is_none() {
            return Err(Error::Custom("no lmtd path is specified".to_string()));
        }
...
impl Lmtd {
    pub fn new(args: LmtdConfig) -> Self {
        Self {
    ...
    pub fn configure(&self, config: LmtdConfig) -> Result<()> {
    ...
    pub fn start(&self) -> Result<()> {
        ...
        if let Some(process) = process {
            if process.is_running() {
                return Err(Error::Custom("Lapis Monetae node is already running.".to_string()));
            }
        }
        ...
        // let options = LmtdOptions::new(path,network)?;
    ...
    pub async fn version(&self) -> Result<String> {
        ...
        if let Some(path) = path {
            ...
            Ok("LMTD binary is not configured. Please use 'node select' command.".to_string())
        }
    }
}
...
pub trait LmtdCtl {
    ...
    async fn configure(&self, config: LmtdConfig) -> Result<()>;

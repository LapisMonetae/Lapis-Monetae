...
pub use lmtd::{Lmtd, LmtdConfig, LmtdCtl};
...
pub static LOCATIONS: &[&str] = &[
    ...
    "../target/release",
    "../target/debug",
    "../../lmt-cpu-miner/target/debug",
    "../../lmt-cpu-miner/target/release",
    "bin/windows-x64",
    "bin/linux-ia32",
    "bin/linux-x64",
    "bin/linux-arm64",
...
#[derive(Default)]
pub struct Daemons {
    pub lmtd: Option<Arc<dyn LmtdCtl + Send + Sync + 'static>>,
    // pub lmtd_automute : Arc<
    pub cpu_miner: Option<Arc<dyn CpuMinerCtl + Send + Sync + 'static>>,
}

impl Daemons {
    pub fn new() -> Self {
        Self { lmtd: None, cpu_miner: None }
    }

    pub fn with_lmtd(mut self, lmtd: Arc<dyn LmtdCtl + Send + Sync + 'static>) -> Self {
        self.lmtd = Some(lmtd);
        self
    }
    ...
    pub fn lmtd(&self) -> Arc<dyn LmtdCtl + Send + Sync + 'static> {
        self.lmtd.as_ref().expect("accessing Daemons::lmtd while lmtd option is None").clone()
    }

    pub fn try_lmtd(&self) -> Option<Arc<dyn LmtdCtl + Send + Sync + 'static>> {
        self.lmtd.clone()
    }

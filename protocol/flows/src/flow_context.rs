...
use lmt_connectionmanager::ConnectionManager;
use lmt_consensus_core::block::Block;
use lmt_consensus_core::config::Config;
use lmt_consensus_core::errors::block::RuleError;
use lmt_consensus_core::tx::{Transaction, TransactionId};
...
use lmt_consensusmanager::{BlockProcessingBatch, ConsensusInstance, ConsensusManager, ConsensusProxy, ConsensusSessionOwned};
use lmt_core::{
    debug, info,
    lmtd_env::{name, version},
    task::tick::TickService,
};
use lmt_core::{time::unix_now, warn};
use lmt_hashes::Hash;
use lmt_mining::mempool::tx::{Orphan, Priority};
use lmt_mining::{manager::MiningManagerProxy, mempool::tx::RbfPolicy};
use lmt_notify::notifier::Notify;
use lmt_p2p_lib::{
    common::ProtocolError,
    convert::model::version::Version,
    make_message,
    pb::{lmtd_message::Payload, InvRelayBlockMessage},
    ConnectionInitializer, Hub, LmtdHandshake, PeerKey, PeerProperties, Router,
};
...
impl FlowContext {
    ...
    ///
    /// _GO-LMTD: OnNewBlock + broadcastTransactionsAfterBlockAdded_
    pub async fn on_new_block(
        ...
        blocks.sort_by_key(|entry| entry.0.header.blue_work);

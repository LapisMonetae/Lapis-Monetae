use crate::{
    ...
    handshake::LmtdHandshake,
    pb::{self, VersionMessage},
    IncomingRoute, LmtdMessagePayloadType, Router,
};
use lmt_core::{debug, time::unix_now, trace, warn};
use std::sync::Arc;
...
impl EchoFlow {
    pub async fn register(router: Arc<Router>) {
        ...
        let receiver = router.subscribe(vec![
            LmtdMessagePayloadType::Addresses,
            LmtdMessagePayloadType::Block,
            LmtdMessagePayloadType::Transaction,
            LmtdMessagePayloadType::BlockLocator,
            LmtdMessagePayloadType::RequestAddresses,
            LmtdMessagePayloadType::RequestRelayBlocks,
            LmtdMessagePayloadType::RequestTransactions,
            LmtdMessagePayloadType::IbdBlock,
            LmtdMessagePayloadType::InvRelayBlock,
            LmtdMessagePayloadType::InvTransactions,
            LmtdMessagePayloadType::Ping,
            LmtdMessagePayloadType::Pong,
            // LmtdMessagePayloadType::Verack,
            // LmtdMessagePayloadType::Version,
            // LmtdMessagePayloadType::Ready,
            LmtdMessagePayloadType::TransactionNotFound,
            LmtdMessagePayloadType::Reject,
            LmtdMessagePayloadType::PruningPointUtxoSetChunk,
            LmtdMessagePayloadType::RequestIbdBlocks,
    ...
    async fn call(&self, msg: pb::LmtdMessage) -> bool {
        // echo
...
impl ConnectionInitializer for EchoFlowInitializer {
    async fn initialize_connection(&self, router: Arc<Router>) -> Result<(), ProtocolError> {
        //
        // Example code to illustrate Lapis Monetae P2P handshaking
        //

        // Build the handshake object and subscribe to handshake messages
        let mut handshake = LmtdHandshake::new(&router);
...
mod tests {
    ...
    use lmt_core::debug;
    use lmt_utils::networking::NetAddress;

    #[tokio::test]
    async fn test_handshake() {
        lmt_core::log::try_init_logger("debug");

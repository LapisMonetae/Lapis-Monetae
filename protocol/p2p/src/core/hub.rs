use crate::{common::ProtocolError, pb::LmtdMessage, ConnectionInitializer, Peer, Router};
use lmt_core::{debug, info, warn};
...
impl Hub {
    ...
    pub async fn send(&self, peer_key: PeerKey, msg: LmtdMessage) -> Result<bool, ProtocolError> {
    ...
    pub async fn broadcast(&self, msg: LmtdMessage) {
    ...
    pub async fn broadcast_to_some_peers(&self, msg: LmtdMessage, num_peers: usize) {
        assert!(num_peers > 0);
    ...
    pub async fn broadcast_many(&self, msgs: Vec<LmtdMessage>) {
        if msgs.is_empty() {
            return;
        }
        let peers = self.peers.read().values().cloned().collect::<Vec<_>>();

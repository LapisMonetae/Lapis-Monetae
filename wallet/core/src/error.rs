...
use lmt_bip32::Error as BIP32Error;
use lmt_consensus_core::sign::Error as CoreSignError;
use lmt_rpc_core::RpcError as LmtRpcError;
use lmt_wrpc_client::error::Error as LmtWorkflowRpcError;
...
pub enum Error {
    ...
    #[error(transparent)]
    WalletKeys(#[from] lmt_wallet_keys::error::Error),

    #[error("please select an account")]
    AccountSelection,

    #[error("{0}")]
    LmtRpcClientResult(#[from] LmtRpcError),

    #[error("wRPC -> {0}")]
    RpcError(#[from] RpcError),

    #[error("Wallet wRPC -> {0}")]
    LmtWorkflowRpcError(#[from] LmtWorkflowRpcError),
    ...
    #[error("{0}")]
    NetworkType(#[from] lmt_consensus_core::network::NetworkTypeError),

    #[error("{0}")]
    NetworkId(#[from] lmt_consensus_core::network::NetworkIdError),
    ...
    #[error(transparent)]
    Address(#[from] lmt_addresses::AddressError),
    ...
    #[error(transparent)]
    ScriptBuilderError(#[from] lmt_txscript::script_builder::ScriptBuilderError),
    ...
    #[error(transparent)]
    ConsensusClient(#[from] lmt_consensus_client::error::Error),

    #[error(transparent)]
    ConsensusWasm(#[from] lmt_consensus_wasm::error::Error),
    ...
    #[error(transparent)]
    MultisigCreateError(#[from] lmt_txscript::MultisigCreateError),
    ...
    #[error(transparent)]
    Metrics(#[from] lmt_metrics_core::error::Error),

    #[error("Connected node is not synced")]
    NotSynced,
    #[error(transparent)]
    Pskt(#[from] lmt_wallet_pskt::error::Error),

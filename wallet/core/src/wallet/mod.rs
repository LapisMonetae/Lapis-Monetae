...
use lmt_bip32::{ExtendedKey, Language, Mnemonic, Prefix as KeyPrefix, WordCount};
use lmt_notify::{
    listener::ListenerId,
    scope::{Scope, VirtualDaaScoreChangedScope},
};
use lmt_wallet_keys::xpub::NetworkTaggedXpub;
use lmt_wrpc_client::{LmtRpcClient, Resolver, WrpcEncoding};
...
impl Wallet {
    ...
    pub fn try_with_wrpc(store: Arc<dyn Interface>, resolver: Option<Resolver>, network_id: Option<NetworkId>) -> Result<Wallet> {
        let rpc_client =
            Arc::new(LmtRpcClient::new_with_args(WrpcEncoding::Borsh, Some("wrpc://127.0.0.1:17110"), resolver, network_id, None)?);
    ...
    pub fn try_wrpc_client(&self) -> Option<Arc<LmtRpcClient>> {
        self.try_rpc_api().and_then(|api| api.clone().downcast_arc::<LmtRpcClient>().ok())
    }

    pub fn wrpc_client(&self) -> Arc<LmtRpcClient> {
        self.try_rpc_api().and_then(|api| api.clone().downcast_arc::<LmtRpcClient>().ok()).unwrap()
    }
    ...
    pub fn address_prefix(&self) -> Result<lmt_addresses::Prefix> {
    ...
    pub async fn import_lmt_wallet_golang_single_v1<T: AsRef<[u8]>>(
        self: &Arc<Wallet>,
        ...
        let prefix = file.xpublic_key.split_at(lmt_bip32::Prefix::LENGTH).0;
        let prefix = lmt_bip32::Prefix::try_from(prefix)?;
    ...
    pub async fn import_lmt_wallet_golang_single_v0<T: AsRef<[u8]>>(
        self: &Arc<Wallet>,
        import_secret: &Secret,
        wallet_secret: &Secret,
    ...
    pub async fn import_lmt_wallet_golang_multisig_v0<T: AsRef<[u8]>>(
        self: &Arc<Wallet>,
        import_secret: &Secret,
        wallet_secret: &Secret,
        ...
        let prefix = first_pub_key.split_at(lmt_bip32::Prefix::LENGTH).0;
        let prefix = lmt_bip32::Prefix::try_from(prefix)?;
    ...
    pub async fn import_lmt_wallet_golang_multisig_v1<T: AsRef<[u8]>>(
        self: &Arc<Wallet>,
        import_secret: &Secret,
        wallet_secret: &Secret,
        ...
        all_pub_keys.sort_unstable_by(|left, right| {
            left.split_at(lmt_bip32::Prefix::LENGTH).1.cmp(right.split_at(lmt_bip32::Prefix::LENGTH).1)
        });
    ...
    pub async fn import_multisig_with_mnemonic(
        ...
        let mut additional_xpub_keys = additional_xpub_keys
            ...
            .collect::<Result<Vec<_>, lmt_bip32::Error>>()?;
...
mod test {
    ...
    use lmt_bip32::{ChildNumber, ExtendedPrivateKey, SecretKey};
    use lmt_consensus_core::subnets::SUBNETWORK_ID_NATIVE;
    use lmt_consensus_wasm::{sign_transaction, SignableTransaction, Transaction, TransactionInput, TransactionOutput};
    ...
    async fn wallet_test() -> Result<()> {
        ...
        let derivation_path =
            gen1::WalletDerivationManager::build_derivate_path(false, 0, None, Some(lmt_bip32::AddressType::Receive))?;
